...
class CILWriter : public FunctionPass {
...
public:
    ...
    CILWriter();
    CILWriter(DataLayout *td, const std::string &_classname, const std::string& outfile, const GenerationOpts *_opts);
    ~CILWriter();
...
private:
    ...
    friend struct llvm::AnalysisInfoMixin<CILWriter>;

    // ILWriter specific (non-LLVM) stuff
    /** The output stream */
...
//------------------------------------------------------------------------------
class ILWriterPrinter
    : public llvm::PassInfoMixin<ILWriterPrinter>
{
public:
    explicit ILWriterPrinter(llvm::raw_ostream &OutS) : OS(OutS) {}
...
// Legacy PM interface
//------------------------------------------------------------------------------
struct LegacyILWriter : public llvm::ModulePass
{
    static char ID;
    LegacyILWriter() : llvm::ModulePass(ID) {}
